...
class StockSpanner {
    ...
    //store in the removed index the difference between current index and the index of the removed
    ...
    public int next(int price) {
        ...
        // the top to reach a greater number (because if this number is higher
        // than the last number at the top, then it means that for the current number
        // to find a higher element, it must also go through those days)
        //
        while(!mono.isEmpty() && mono.peek()[0] < price){
            //since we are keeping a monotonic stack,
            //when finding a price greater than the top
            //remove element and add to answer its ans

            ans += mono.pop()[1];
        }

        //add current [price, the number of days needed at last point] to the stack
